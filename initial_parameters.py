...
def o3(gmodel, lam, cube, SN, c, results_dir, aut_ini):
    ...
    if aut_ini == 'yes':
        lam_i = (np.abs(lam - ((((params['vel1'] * params['lam01']) + (params['vel2'] * params['lam02'])) / 2.) / c + (
                    (params['lam01'] + params['lam02']) / 2.) - 70.))).argmin()
        lam_f = (np.abs(lam - ((((params['vel1'] * params['lam01']) + (params['vel2'] * params['lam02'])) / 2.) / c + (
                    (params['lam01'] + params['lam02']) / 2.) + 70.))).argmin()
...
def o3_cons(gmodel, lam, cube, SN, c, results_dir, aut_ini):
    ...
    if aut_ini == 'yes':
        lam_i = (np.abs(lam - ((((params['vel'] * params['lam01']) + (params['vel'] * params['lam02'])) / 2.) / c + (
                    (params['lam01'] + params['lam02']) / 2.) - 70.))).argmin()
        lam_f = (np.abs(lam - ((((params['vel'] * params['lam01']) + (params['vel'] * params['lam02'])) / 2.) / c + (
                    (params['lam01'] + params['lam02']) / 2.) + 70.))).argmin()
...
def hb_ha_n2_cons(gmodel, lam, cube, SN, c, results_dir, aut_ini):
    ...
    if aut_ini == 'yes':
        ...
        if SL_flag == 'yes':
            f_res = np.concatenate((cube[0].data[lam_i_hb:lam_f_hb, :, :] - cube[5].data[lam_i_hb:lam_f_hb, :, :],
                                    cube[0].data[lam_i_ha:lam_f_ha, :, :] - cube[5].data[lam_i_ha:lam_f_ha, :, :]))
        if SL_flag == 'no':
            f_res = np.concatenate((cube[0].data[lam_i_hb:lam_f_hb, :, :], cube[0].data[lam_i_ha:lam_f_ha, :, :]))
