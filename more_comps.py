...
def two_gaussians_cons_o3(naxis1, naxis2, result_list, results_dir, lam_r, params):
    ...
    for i in np.arange(naxis1):
        for j in np.arange(naxis2):
            ...
            gaussian_region = (results[j, i].best_fit > results[j, i].params['a'].value + (
                    lam_r * results[j, i].params['b'].value)) * ~np.isclose(results[j, i].best_fit,
                                                                            results[j, i].params['a'].value + (
                                                                                    lam_r *
                                                                                    results[j, i].params[
                                                                                        'b'].value), rtol=1e-4)
            ...
            if (any(r_max[k] == r_max[k + 1] == True for k in range(len(r_max) - 1)) | any(
                    r_min[k] == r_min[k + 1] == True for k in range(len(r_min) - 1))) and (
                    results[j, i].params['flux'].stderr):
                if (results[j, i].params['flux'].stderr / results[j, i].params['flux'].value < 0.5) & (
                        results[j, i].params['sig'].stderr < 20) & (results[j, i].params['vel'].stderr < 20):
                    ncomps_flag[j, i] = 1
                    bic_flag[j, i] = 1
    ...
    nparams.add('flux_delta', value=init_params['flux_delta'], min=init_params['flux_delta_min'],
                max=init_params['flux_delta_max'], vary=True)
    ...
    nparams.add('sig_delta', value=init_params['sig_delta'], min=init_params['sig_delta_min'],
                max=init_params['sig_delta_max'], vary=True)
    nparams['sig_b'].set(expr='sig_delta*sig')

    nparams['vel'].min = init_params['vel_min']
    nparams['vel'].max = init_params['vel_max']
    nparams.add('vel_delta', value=init_params['vel_delta'], min=init_params['vel_delta_min'],
                max=init_params['vel_delta_max'], vary=True)
    ...
    with mp.Pool(n_proc) as pool:
        results_k = pool.starmap(kloop, zip(
            (results[y_mask[k], x_mask[k]].data, lam_r, nparams, k, len(y_mask)) for k in np.arange(len(y_mask))))
...
def three_gaussians_cons(naxis1, naxis2, result_list, results_dir, lam_r, params):
    ...
    ncomps_flag = np.zeros([naxis2, naxis1]) + 1
    bic_flag = np.zeros([naxis2, naxis1]) + 1
    ...
    #         # res_max_lim = 5 * np.std(np.array(results[j, i].data)[np.r_[0:20, len(lam_r) - 20:len(lam_r)]])
    #         # res_min_lim = -5 * np.std(np.array(results[j, i].data)[np.r_[0:20, len(lam_r) - 20:len(lam_r)]])
    #         res_lim = 5 * np.std(np.array(results[j, i].data)[np.r_[0:20, len(lam_r) - 20:len(lam_r)]])
    #
    #         # line_peak = np.where(results[j, i].best_fit == np.nanmax(results[j, i].best_fit))[0][0]
    #
    #         gaussian_region = (results[j, i].best_fit > results[j, i].params['a'].value +
    #                            (lam_r * results[j, i].params['b'].value)) * \
    #                           ~np.isclose(results[j, i].best_fit, results[j, i].params['a'].value +
    #                                       (lam_r * results[j, i].params['b'].value), rtol=1e-4)
    #
    ...
    #         r_max = ((results[j, i].residual > res_lim) * gaussian_region)
    #         r_min = ((results[j, i].residual < -res_lim) * gaussian_region)
    #
    #         # if (any(r_max[k] == r_max[k + 1] is True for k in range(len(r_max) - 1))) |
    #         #         any(r_min[k] == r_min[k + 1] is True for k in range(len(r_min) - 1))):
    #         if any(r_max) | any(r_min):
    #             # if (results[j,i].params['flux'].stderr):
    #             # if (results[j, i].params['sig'].stderr < 35) & (results[j, i].params['vel'].stderr < 35) & (
    #             #         results[j, i].params['sig'].value > 120):
    #             ncomps_flag[j, i] = 1
    #             bic_flag[j, i] = 1
    ...
    nparams['vel'].min = init_params['vel'] + init_params['vel_min']
    nparams['vel'].max = init_params['vel'] + init_params['vel_max']
    # nparams['vel'].min = init_params['vel_min']
    # nparams['vel'].max = init_params['vel_max']
    nparams.add('vel_delta', value=init_params['vel_delta'], min=init_params['vel_delta_min'],
                max=init_params['vel_delta_max'], vary=True)
    ...
    with mp.Pool(n_proc) as pool:
        results_k = pool.starmap(kloop, zip(
            (results[y_mask[k], x_mask[k]].data, lam_r, nparams, y_mask[k], x_mask[k]) for k in np.arange(len(y_mask))))

    for k in np.arange(len(y_mask)):
        amp = results_k[k].params['flux'].value / \
              (np.sqrt(2 * np.pi) * (np.sqrt((results_k[k].params['sig'].value ** 2) +
                                             (inst_sigma_red ** 2)) * results_k[k].params['lam02'].value / c))
        amp_b = results_k[k].params['flux_b'].value / \
                (np.sqrt(2 * np.pi) * (np.sqrt((results_k[k].params['sig_b'].value ** 2) +
                                               (inst_sigma_red ** 2)) * results_k[k].params['lam02'].value / c))

        res_lim = 5 * np.std(np.array(results[y_mask[k], x_mask[k]].data)[np.r_[0:20, len(lam_r) - 20:len(lam_r)]])
        if (results_k[k].bic < results[y_mask[k], x_mask[k]].bic):
            results[y_mask[k], x_mask[k]] = results_k[k]
            bic_flag[y_mask[k], x_mask[k]] = 2
...
def hb_ha_n2_gaussians_cons(naxis1, naxis2, result_list, results_dir, lam_r, params):
    ...
    for i in np.arange(naxis1):
        for j in np.arange(naxis2):
            ...
            res_max_lim = 5 * np.std(np.array(results[j, i].data)[np.r_[int(len(lam_r) / 2):int(len(lam_r) / 2) + 20,
                                                                  len(lam_r) - 20:len(lam_r)]])
            res_min_lim = -5 * np.std(np.array(results[j, i].data)[np.r_[int(len(lam_r) / 2):int(len(lam_r) / 2) + 20,
                                                                   len(lam_r) - 20:len(lam_r)]])
            ...
            gaussian_region = (results[j, i].best_fit > results[j, i].params['a_ha'].value + (
                    lam_r * results[j, i].params['b_ha'].value)) * ~np.isclose(results[j, i].best_fit,
                                                                               results[j, i].params[
                                                                                   'a_ha'].value + (lam_r * results[
                                                                                   j, i].params['b_ha'].value),
                                                                               rtol=1e-4)
