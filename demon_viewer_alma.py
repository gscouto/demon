...
dir = '/home/guilherme/oso/vales_sample2/EL_measurements/GAMA324842/co3-2_2021-08-20_12.26.14/'
...
def update_plot():
    ...
    if ('Hb_Ha_N2' in dir) or ('hb_ha_n2' in dir):
        ...
        ax1.plot(fit_results['freq'][0:int(len(fit_results['freq'])/2)],fit_results['data'][0:int(len(fit_results['freq'])/2),y,x],'.')
        ax1.plot(fit_results['freq'][0:int(len(fit_results['freq'])/2)],fit_results['data'][0:int(len(fit_results['freq'])/2),y,x],color='black')
        ax1.plot(fit_results['freq'][0:int(len(fit_results['freq'])/2)],fit_results['model'][0:int(len(fit_results['freq'])/2),y,x],color='red')
        if '2g' in dir:
            ax1.plot(fit_results['freq'][0:int(len(fit_results['freq'])/2)],fit_results['model_2g_n'][0:int(len(fit_results['freq'])/2),y,x],'--',color='green',linewidth=0.7)
            ax1.plot(fit_results['freq'][0:int(len(fit_results['freq'])/2)],fit_results['model_2g_b'][0:int(len(fit_results['freq'])/2),y,x],'--',color='blue',linewidth=0.7)
        ax1.xaxis.set_major_formatter( NullFormatter() )
        ax1.grid(True,alpha=0.3)

        ax2.clear()
        ax2.plot(fit_results['freq'][0:int(len(fit_results['freq'])/2)],fit_results['residual'][0:int(len(fit_results['freq'])/2),y,x])
        ax2.grid(True,alpha=0.3)
        
        ax3.clear()
        ax3.plot(fit_results['freq'][int(len(fit_results['freq'])/2):],fit_results['data'][int(len(fit_results['freq'])/2):,y,x],'.')
        ax3.plot(fit_results['freq'][int(len(fit_results['freq'])/2):],fit_results['data'][int(len(fit_results['freq'])/2):,y,x],color='black')
        ax3.plot(fit_results['freq'][int(len(fit_results['freq'])/2):],fit_results['model'][int(len(fit_results['freq'])/2):,y,x],color='red')
        if '2g' in dir:
            ax3.plot(fit_results['freq'][int(len(fit_results['freq'])/2):],fit_results['model_2g_n'][int(len(fit_results['freq'])/2):,y,x],'--',color='green',linewidth=0.7)
            ax3.plot(fit_results['freq'][int(len(fit_results['freq'])/2):],fit_results['model_2g_b'][int(len(fit_results['freq'])/2):,y,x],'--',color='blue',linewidth=0.7)
        ax3.xaxis.set_major_formatter( NullFormatter() )
        ax3.grid(True,alpha=0.3)
        ax3.yaxis.tick_right()

        ax4.clear()
        ax4.plot(fit_results['freq'][int(len(fit_results['freq'])/2):],fit_results['residual'][int(len(fit_results['freq'])/2):,y,x])
        ...
        ax1.clear()
        ax1.set_title('x = '+str(x)+', y = '+str(y))
        ax1.plot(fit_results['freq'][:],fit_results['data'][:,y,x],'.')
        ax1.plot(fit_results['freq'][:],fit_results['data'][:,y,x],color='black')
        ax1.plot(fit_results['freq'][:],fit_results['model'][:,y,x],color='red')
        if '2g' in dir:
            ax1.plot(fit_results['freq'][:],fit_results['model_2g_n'][:,y,x],'--',color='green',linewidth=0.7)
            ax1.plot(fit_results['freq'][:],fit_results['model_2g_b'][:,y,x],'--',color='blue',linewidth=0.7)
        ax1.xaxis.set_major_formatter( NullFormatter() )
        ax1.grid(True,alpha=0.3)
        
        ax2.clear()
        ax2.plot(fit_results['freq'][:],fit_results['residual'][:,y,x])
        ax2.grid(True,alpha=0.3)
