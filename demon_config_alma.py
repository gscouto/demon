...
cube_path = '/home/guilherme/oso/vales_sample2/ALMA/'         # directory path to where the cubes to be fitted are located
...
lines_to_fit ='co3-2'   # which (set of) lines will be measured?
#options: 'ha_n2', 'ha_n2_cons', 'hb_ha_n2_cons', 'hb', 'o3', 'o3_cons', 's2', 's2_cons', 'o1', 'o1_cons', 'co3-2', 'all' ['ha_n2'+'hb'+'o3'+'s2'+'o1'], 'all_cons' ['hb_ha_n2_cons'+'o3_cons'+'s2_cons'+'o1_cons']
...
SL_flag = 'no'              # is the datacube file a product of the STARLIGHT code (after stellar continuum fitting)?
...
ids = ['GAMA324842']     # names of the galaxies to be fitted (they must match the name of the datacube file, ex: 'NGC1097'+.fits)
...
freqi_SN = 291.5            # lower wavelenght to calculate signal to noise ratio
freqf_SN = 292.0            # upper wavelenght to calculate signal to noise ratio

freqi_cont1 = 291.0         # lower wavelenght to first window of the local continuum / this is used only to calculate the S/N ratio
freqf_cont1 = 291.3         # upper wavelenght to first window of the local continuum / this is used only to calculate the S/N ratio

freqi_cont2 = 292.0         # upper wavelenght to second window of the local continuum / this is used only to calculate the S/N ratio
freqf_cont2 = 292.2         # upper wavelenght to second window of the local continuum / this is used only to calculate the S/N ratio

##### FITTING INITIAL PARAMETERS AND LIMITS

init_params = {

'a' : 0.,                  # first parameter of the local continuum fitted in the emission-line (a+b*x, x = wavelength vector)
'b' : 0.,                   # second parameter of the local continuum fitted in the emission-line (a+b*x, x = wavelength vector)

'flux' : 10.,              # initial guess (IG) for the integrated flux of the (main) emission line
